...
std::string ResolveConfigPath(const std::string &config_file,
  ...
  if (configured_path.empty() ||
      configured_path.front() == '/' ||
      (configured_path.size() > 1 && configured_path[1] == ':')) {
    return configured_path;
  }
  const size_t separator = config_file.find_last_of("/\\");
  if (separator == std::string::npos) {
    return configured_path;
  }
  return config_file.substr(0, separator + 1) + configured_path;
}
...
bool LoadConfig(const std::string &config_file,hobot::dnn_node::racing_obstacle_detection::PTQYolo5Config &yolo5_config) {
  ...
  if (!document.HasMember("class_num") ||
      InitClassNum(document["class_num"].GetInt(), yolo5_config) < 0) {
    return false;
  }
  if (!document.HasMember("cls_names_list") ||
      InitClassNames(
          ResolveConfigPath(config_file, document["cls_names_list"].GetString()),
          yolo5_config) < 0) {
    return false;
  }
  if (!document.HasMember("strides") || !document["strides"].IsArray() ||
      document["strides"].Size() != 3) {
    return false;
  }
  yolo5_config.strides.clear();
  for (const auto &stride : document["strides"].GetArray()) {
    yolo5_config.strides.push_back(stride.GetInt());
  }
  if (!document.HasMember("anchors_table") ||
      !document["anchors_table"].IsArray() ||
      document["anchors_table"].Size() != yolo5_config.strides.size()) {
    return false;
  }
  yolo5_config.anchors_table.clear();
  for (const auto &layer : document["anchors_table"].GetArray()) {
    if (!layer.IsArray() || layer.Empty()) {
      return false;
    }
    std::vector<std::pair<double, double>> anchors;
    for (const auto &anchor : layer.GetArray()) {
      if (!anchor.IsArray() || anchor.Size() != 2) {
        return false;
      }
      anchors.emplace_back(anchor[0].GetDouble(), anchor[1].GetDouble());
    }
    yolo5_config.anchors_table.push_back(std::move(anchors));
  }
  ...
  return !yolo5_config.class_names.empty() &&
         static_cast<int>(yolo5_config.class_names.size()) == yolo5_config.class_num;
}
...
ObstacleDetectionNode::ObstacleDetectionNode(const std::string& node_name,
  ...
  if (Init() != 0 ||
    ...
    throw std::runtime_error("Obstacle detection node initialization failed");
  }
  if (!LoadConfig(config_file_, yolo5_config_)) {
    RCLCPP_ERROR(rclcpp::get_logger("ObstacleDetectionNode"),
                 "Invalid YOLO configuration: %s", config_file_.c_str());
    throw std::runtime_error("Invalid YOLO configuration");
  }
...
int ObstacleDetectionNode::SetNodePara() {
  ...
  if (document.HasMember("model_file")) {
    model_file = ResolveConfigPath(config_file_, document["model_file"].GetString());
  }
